...
const Pasarela = ({ DatosPersonales, handleClickSiguiente, Tarjeta, Diferido }) => {
    const [trx, setTrx] = useState();
    const Persona = {
        "dni": DatosPersonales["cedula"],
        "address": DatosPersonales["ciudad"].Titulo,
        "phone": DatosPersonales["telefono"],
        "names": DatosPersonales["nombres"],
        "email": DatosPersonales["correo"],
        "promo": DatosPersonales["promo"]
    };
    const Producto = {
        "precio_producto": "78",
        "titulo": "Suscripción CPN - VisitaEcuador.com 1 año"
    };

    const datafastController = new DatafastController({ persona: Persona, product: Producto });
    const [html, setHtml] = useState();
    const cardType = Tarjeta;
    ...
    const getTemplateDiferido = (diferido, interes) => {
        ...
        async function fetchData() {
            try {
                ...
                if (result) {
                    ...
                    template = template.replace("{{DEFERRED}}", getTemplateDiferido(Diferido["tipo"],Diferido["meses"]));
                    setHtml(template);
    ...
    return (
        <div>
            ...
            <div className='flex justify-center mb-14'>
                <button className=' w-32 py-2 bg-orangeCPN-600 text-white rounded-full text-lg mb-6' onClick={()=>handleClickButton()}>Anterior</button>
            </div>
        </div>
    );
}
